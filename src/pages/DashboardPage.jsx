...
export default function DashboardPage({ store, session }) {
  ...
  const stats = useMemo(() => {
    ...
    const last7 = Array.from({length:7}, (_,i) => {
      ...
      return { date: d, label: t('{n}日', { n: d.getDate() }), revenue: dayOrders.reduce((s,o)=>s+o.total,0), count: dayOrders.length }
    })
  ...
  return (
    <div style={ds.root}>
      ...
      <div style={ds.hero} className="animate-up">
        ...
        {openShift && (
          <div style={ds.shiftBadge}>
            ...
            {t('營業中 · 開班')} {new Date(openShift.openTime).toLocaleTimeString(getLocale(),{hour:'2-digit',minute:'2-digit'})}
          </div>
        )}
      </div>
      ...
      <div style={ds.kpiGrid}>
        <KpiCard icon={DollarSign} color="gold" label={t('今日營收')} value={fmtMoney(todayRevenue)}
          delta={stats.revDelta} subtitle={t('比昨日 {pct}%', { pct: `${stats.revDelta >= 0 ? '+' : ''}${stats.revDelta.toFixed(1)}` })} idx={0}/>
        <KpiCard icon={ShoppingBag} color="blue" label={t('今日訂單')}
          value={stats.ordersToday.length} subtitle={t('平均 {amt}/單', { amt: fmtMoney(stats.ordersToday.length ? Math.round(todayRevenue / stats.ordersToday.length) : 0) })} idx={1}/>
        <KpiCard icon={TrendingUp} color="green" label={t('今日毛利')}
          value={fmtMoney(todayProfit)} subtitle={t('毛利率 {pct}%', { pct: todayRevenue > 0 ? (todayProfit/todayRevenue*100).toFixed(1) : 0 })} idx={2}/>
        <KpiCard icon={Users} color="teal" label={t('會員 / 商品')} value={`${members.length} / ${products.length}`} subtitle={t('會員數 / 商品數')} idx={3}/>
      </div>

      {/* 30 天指標 row */}
      <div style={ds.kpiGrid}>
        <KpiCard icon={BarChart2} color="purple" label={t('30 天客單價')}
          value={fmtMoney(Math.round(analytics30d.avgTicket.avg))}
          subtitle={t('{n} 筆訂單', { n: analytics30d.avgTicket.count })} idx={0}/>
        <KpiCard icon={Percent} color="green" label={t('30 天毛利率')}
          value={`${analytics30d.profit.marginRate.toFixed(1)}%`}
          subtitle={t('毛利 {amt}', { amt: fmtMoney(Math.round(analytics30d.profit.profit)) })} idx={1}/>
        <KpiCard icon={Truck} color="amber" label={t('待補貨')}
          value={analytics30d.reorderList.length}
          subtitle={analytics30d.reorderList.length > 0 ? t('點進貨管理 ➜') : t('庫存充足')} idx={2}/>
        <KpiCard icon={Clock} color="red" label={t('即將/已過期')}
          value={`${analytics30d.soon.length} / ${analytics30d.expired.length}`}
          subtitle={t('7天內 / 已過期')} idx={3}/>
      </div>
      ...
      <div style={ds.card}>
        <div style={ds.cardHead}>
          <div style={{display:'flex', alignItems:'center', gap:8}}>
            ...
            <span style={{fontWeight:600}}>{t('會員結構（30 天）')}</span>
          </div>
          <span style={{fontSize:11, color:'var(--text-tertiary)'}}>
            {t('會員占 {pct}% 營收', { pct: analytics30d.segmentation.memberRevenue + analytics30d.segmentation.anonRevenue > 0
        ...
        <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(120px, 1fr))', gap:8, marginTop:8}}>
          {[
            ...
            <div key={tag} style={{padding:'10px 12px', background:'var(--bg-overlay)', borderRadius:8, borderTop:`2px solid ${color}`}}>
              <div style={{fontSize:11, color:'var(--text-tertiary)', marginBottom:3}}>{t(tag)}</div>
      ...
      <div style={ds.card}>
        <div style={ds.cardHead}>
          <div style={{display:'flex', alignItems:'center', gap:8}}>
            ...
            <span style={{fontWeight:600}}>{t('今日銷售目標')}</span>
          </div>
          {!editingGoal ? (
            <button className="btn btn-ghost btn-sm" onClick={()=>setEditingGoal(true)}>{t('設定目標')}</button>
          ) : (
            <div style={{display:'flex', gap:6}}>
              ...
              <button className="btn btn-primary btn-sm" onClick={handleSaveGoal}>{t('儲存')}</button>
              <button className="btn btn-ghost btn-sm" onClick={()=>{setEditingGoal(false);setGoalInput(String(salesGoal))}}>{t('取消')}</button>
            </div>
          )}
        </div>
        {salesGoal > 0 ? (
          <>
            <div style={{display:'flex', justifyContent:'space-between', alignItems:'baseline', marginBottom:6}}>
              <span style={{fontFamily:'var(--font-mono)', fontSize:18, fontWeight:600}}>
                {fmtMoney(todayRevenue)} <span style={{fontSize:12, color:'var(--text-tertiary)', fontWeight:400}}>/ {salesGoal.toLocaleString(getLocale())}</span>
              </span>
            ...
            <div style={{fontSize:11, color:'var(--text-tertiary)', marginTop:6}}>
              {goalPct >= 100 ? t('🎉 已達標！') : t('還差 {amt}', { amt: fmtMoney(Math.max(0, salesGoal - todayRevenue)) })}
            </div>
          </>
        ) : (
          <div style={{color:'var(--text-tertiary)', fontSize:13, textAlign:'center', padding:'12px 0'}}>{t('尚未設定目標')}</div>
        )}
      </div>
      ...
      <div style={ds.row2}>
        ...
        <div style={ds.card}>
          <div style={ds.cardHead}>
            <span style={{fontWeight:600}}>{t('近 7 天營收')}</span>
          </div>
        ...
        <div style={ds.card}>
          <div style={ds.cardHead}>
            <div style={{display:'flex', alignItems:'center', gap:8}}>
              ...
              <span style={{fontWeight:600}}>{t('今日熱銷')}</span>
            </div>
          </div>
          {stats.topItems.length === 0 ? (
            <div style={ds.empty}>{t('今日尚無銷售')}</div>
            ...
            <div key={i} style={ds.topRow}>
              ...
              <div style={{flex:1, minWidth:0}}>
                ...
                <div style={{fontSize:11, color:'var(--text-tertiary)'}}>{t('賣出 {n} 件', { n: item.qty })}</div>
              </div>
        ...
        <div style={ds.card}>
          <div style={ds.cardHead}>
            <div style={{display:'flex', alignItems:'center', gap:8}}>
              ...
              <span style={{fontWeight:600}}>{t('近 7 天到期')}</span>
            </div>
          </div>
          {stats.expiringSoon.length === 0 ? (
            <div style={ds.empty}>{t('無即期商品')}</div>
