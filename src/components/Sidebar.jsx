...
const NAV = [
  { key:'dashboard',  label:t('首頁'),     Icon:LayoutDashboard, perm:'pos.use', accent:'gold' },
  { key:'pos',        label:t('收銀台'),   Icon:ShoppingCart,    perm:'pos.use', accent:'green' },
  { key:'shifts',     label:t('班別交班'), Icon:Clock,           perm:'pos.use', accent:'teal' },
  { key:'inventory',  label:t('庫存管理'), Icon:Package,         perm:'inventory.view', accent:'blue' },
  { key:'purchase',   label:t('進貨管理'), Icon:Truck,           perm:'purchase.view', accent:'purple' },
  { key:'waste',      label:t('損耗管理'), Icon:Trash2,          perm:'inventory.view', accent:'red' },
  { key:'stocktake',  label:t('每日盤點'), Icon:ClipboardList,   perm:'stocktake.view', accent:'amber' },
  { key:'promotions', label:t('促銷活動'), Icon:Tag,             perm:'promotions.view', accent:'pink' },
  { key:'members',    label:t('會員'),     Icon:Users,           perm:'members.view', accent:'gold' },
  { key:'reports',    label:t('報表分析'), Icon:BarChart2,       perm:'reports.view', accent:'blue' },
  { key:'accounting', label:t('會計帳務'), Icon:BookOpen,        perm:'accounting.view', accent:'teal' },
  { key:'orders',     label:t('顧客點餐'), Icon:Bell,            perm:'pos.use', accent:'amber' },
  { key:'settings',   label:t('設定'),     Icon:Settings,        perm:'settings.view', accent:'gold' },
]
...
export default function Sidebar({ view, setView, session, lowStockCount, todayRevenue, todayOrders, onLogout, pendingOrders = 0, openShift = null }) {
  ...
  const dateStr = now.toLocaleDateString(getLocale(), { month:'numeric', day:'numeric', weekday:'short' })
    ...
    <aside style={s.sidebar}>
      ...
      <div style={s.logoWrap}>
        ...
        <div>
          ...
          <div style={s.logoSub}>{t('智慧雜貨店系統')}</div>
        </div>
      </div>
      ...
      <div style={s.bottom}>
        ...
        {lowStockCount > 0 && (
          <div style={s.alertBox} className="animate-up">
            ...
            <span style={{fontSize:12, color:'var(--amber)', fontWeight:600}}>{t('{n} 項低庫存', {n: lowStockCount})}</span>
          </div>
        )}
        <div style={s.userRow}>
          ...
          <div style={{flex:1, minWidth:0}}>
            ...
            <div style={{fontSize:10, color: role?.color || 'var(--text-tertiary)', fontWeight:600, letterSpacing:'.05em'}}>{role?.label && t(role.label)}</div>
          </div>
          <button className="btn-icon" onClick={onLogout} title={t('登出')}
