...
function formatAgo(iso) {
  if (!iso) return t('尚未同步')
  const diff = Date.now() - new Date(iso).getTime()
  if (diff < 60_000) return t('剛剛')
  const mins = Math.floor(diff / 60_000)
  if (mins < 60) return t('{n} 分鐘前', {n: mins})
...
export default function SyncStatusBadge({ onGoToSettings }) {
  ...
  async function handlePull() {
    if (busy) return
    if (!confirm(t('從雲端拉取會覆蓋本機資料，確認？'))) return
    setBusy('pull')
    setFeedback('')
    try {
      await pullAll(() => {})
      localStorage.setItem('pos_last_sync', new Date().toISOString())
      setFeedback(t('✓ 拉取完成，重新載入...'))
  ...
  if (!enabled) {
    return (
      <button
        ...
        style={styles.box}
        title={t('尚未設定雲端同步，點此前往設定')}
      >
        <CloudOff size={13} style={{color:'var(--text-tertiary)', flexShrink:0}}/>
        <span style={{fontSize:11, color:'var(--text-tertiary)', flex:1}}>{t('未連線雲端')}</span>
      </button>
    )
  }
    ...
    <div style={{...styles.wrap, ...(open ? {gap:6} : {})}}>
      <button onClick={() => setOpen(o => !o)} style={styles.box}>
        ...
        <div style={{flex:1, minWidth:0, textAlign:'left'}}>
          <div style={{fontSize:11, color:'var(--text-secondary)', fontWeight:600}}>
            {busy === 'push' ? t('推送中...') : busy === 'pull' ? t('拉取中...') : t('雲端已連線')}
          </div>
      ...
      {open && !busy && (
        <div style={styles.actions}>
          <button onClick={handlePush} style={{...styles.actionBtn, color:'var(--blue)'}} title={t('把本機改動推到雲端')}>
            <ArrowUp size={12}/>{t('推送')}
          </button>
          <button onClick={handlePull} style={{...styles.actionBtn, color:'var(--amber)'}} title={t('從雲端拉最新（覆蓋本機）')}>
            <ArrowDown size={12}/>{t('拉取')}
          </button>
        </div>
      )}
